...
export function NearbyPanel({ onClose }: { onClose: () => void }) {
  ...
  const [creatorInfo, setCreatorInfo] = useState<CreatorPublicProfile | null>(null);
  const [creatorInfoLoading, setCreatorInfoLoading] = useState(false);
  const [creatorInfoFetched, setCreatorInfoFetched] = useState(false);
  ...
  const handleMoreToggle = async () => {
    if (showMore) { setShowMore(false); return; }
    setShowMore(true);
    if (creatorInfoFetched || !selectedUser?.username) return;
    setCreatorInfoLoading(true);
    setCreatorInfoFetched(true);
    try {
      const info = await getPublicCreatorProfile(selectedUser.username);
      setCreatorInfo(info);
    } catch {
      // 404 = not a creator; creatorInfo stays null
    } finally {
  ...
  return (
    <div className="flex flex-col bg-[#1C1C1E] rounded-t-3xl overflow-hidden" style={{ maxHeight: "80vh" }}>
          ...
          <div className="flex flex-col items-center py-6 text-center gap-4">
            ...
            <div className="w-full px-4">
              <button
                ...
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: showMore ? "#D4007A" : "rgba(255,255,255,0.5)" }}
              >
                {showMore ? "▲ Less" : "▼ More"}
              </button>
              ...
              {showMore && (
                <div className="mt-3 flex flex-col gap-3 pb-2">
                  {creatorInfoLoading ? (
                    ...
                    <>
                      {creatorInfo.callPackages.filter(p => p.is_active).length > 0 && (
                        <div className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                          <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-2">Book a Call</p>
                          {creatorInfo.nextAvailability && (
                            <p className="text-[10px] text-pnp-accent mb-2">
                              Next: {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][creatorInfo.nextAvailability.day_of_week]} · {creatorInfo.nextAvailability.start_time}
                            </p>
                          )}
                          <div className="flex flex-wrap gap-1.5 mb-2.5">
                            {creatorInfo.callPackages.filter(p => p.is_active).map(pkg => (
                      ...
                      {creatorInfo.creator.creator_price_usd > 0 && !creatorInfo.isSubscribed && (
                        <div className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                          ...
                          <p className="text-[10px] text-white/40 mb-2.5">
                            ...
                            {creatorInfo.channels.filter(c => c.access_type !== 'free' && c.price_usd > 0).length > 0 && ` · ${creatorInfo.channels.filter(c => c.access_type !== 'free' && c.price_usd > 0).length} paid channel${creatorInfo.channels.filter(c => c.access_type !== 'free' && c.price_usd > 0).length > 1 ? 's' : ''}`}
                          </p>
                          <button
                          ...
                          >
                            Subscribe · ${creatorInfo.creator.creator_price_usd}/mo
                          </button>
                        </div>
                      )}
            ...
            <button onClick={() => { setView("grid"); setShowMore(false); setCreatorInfo(null); setCreatorInfoFetched(false); }} className="text-sm text-pnp-accent font-medium">Back to grid</button>
          </div>
        )}
      </div>
      ...
      {showBookModal && creatorInfo && (
        <BookCallModal
          creator={{
            id: creatorInfo.creator.id,
            username: creatorInfo.creator.username,
            photo_url: creatorInfo.creator.photo_url,
            creator_type: creatorInfo.creator.creator_type as CreatorCardCreator["creator_type"],
            creator_price_usd: creatorInfo.creator.creator_price_usd,
            bio: creatorInfo.creator.bio,
          }}
          isOnline={false}
          open={showBookModal}
      ...
      {showSubscribeWizard && creatorInfo && (
        <div
        ...
        >
          <div className="w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <CreatorSubscribeWizard
              creatorId={creatorInfo.creator.id}
              creatorName={creatorInfo.creator.first_name}
              priceUsd={creatorInfo.creator.creator_price_usd}
              username={creatorInfo.creator.username}
              onSuccess={() => {
                setCreatorInfo(prev => prev ? { ...prev, isSubscribed: true } : prev);
                setShowSubscribeWizard(false);
              }}
