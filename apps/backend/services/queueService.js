...
function getSubscribeRetargetQueue() {
  if (!_subscribeRetargetQueue) {
    ...
    _subscribeRetargetQueue.on('error', (err) =>
      logger.warn('[BullMQ] subscribe-retarget queue error', { error: err.message })
    );
  }
...
module.exports = {
  ...
  // subscribe-retarget queue (conversion retargeting delayed jobs)
