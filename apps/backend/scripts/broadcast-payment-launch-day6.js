...
/**
 * broadcast-payment-launch-day6.js
 *
 * DAY 6 (final) of the payment-system launch drip.
 * Angle: "One place. Every way to pay."
 * CTA:  /marketing/  (the full grid — wallet, tutorials, subscribe, book).
 *
 * Channels: payment_launch_day6_dm, payment_launch_day6_tg,
 *           payment_launch_day6_push_es, payment_launch_day6_push_en.
 * Dedup table: broadcast_payment_launch_day6.
 *
...
const LOG_TABLE   = 'broadcast_payment_launch_day6';
const DM_CHANNEL  = 'payment_launch_day6_dm';
const TG_CHANNEL  = 'payment_launch_day6_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `💎 Un solo lugar. Cada forma de pagar.

Wallet, Ru$h 💎, suscripciones, llamadas — todo en un solo panel. Elige lo tuyo, todo funciona igual de fácil.

Explora todo 👇
${MARKETING_URL}`;
  }
  return `💎 One place. Every way to pay.

Wallet, Ru$h 💎, subscriptions, calls — all in one panel. Pick your thing, all just as easy.
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `💎 <b>Un solo lugar. Cada forma de pagar.</b>\n\n` +
      `Wallet, Ru$h 💎, suscripciones, llamadas — todo en un solo panel.\n\n` +
      `Explora todo 👇`
    );
  }
  return (
    `💎 <b>One place. Every way to pay.</b>\n\n` +
    `Wallet, Ru$h 💎, subscriptions, calls — all in one panel.\n\n` +
    `Explore everything 👇`
  );
}
...
const TG_BUTTONS = {
  en: [[
    { text: '💎 Explore All', url: MARKETING_URL },
    { text: 'Wallet Tour →',  url: TOUR_URL },
  ]],
  es: [[
    { text: '💎 Explorar Todo', url: MARKETING_URL },
    { text: 'Tour del Wallet →', url: TOUR_URL },
  ]],
};

const PUSH_COPY = {
  es: { title: '💎 Un solo lugar', body: 'Cada forma de pagar · en un panel' },
  en: { title: '💎 One place',      body: 'Every way to pay · one panel' },
};

const PUSH_COMMON = { url: MARKETING_URL, icon: '/icon-192.png', tag: 'payment-launch-day6' };
...
async function sendPushBatch(langKey) {
  ...
  const channel = `payment_launch_day6_push_${langKey}`;
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — Payment System Launch · Day 6 (Explore All)');
  console.log(`  MODE  : ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log(`  Hero  : ${VIDEO_URL}`);
  console.log(`  CTA   : ${MARKETING_URL}`);
  console.log('═══════════════════════════════════════════════════════════════\n');
