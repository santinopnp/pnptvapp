...
const LOG_TABLE   = 'broadcast_dejesusof22_wave5_2026_09_04';
const DM_CHANNEL  = 'dejesusof22_wave5_dm';
const TG_CHANNEL  = 'dejesusof22_wave5_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `🔴 @Dejesusof22 · ÚLTIMO LLAMADO

Se despide pronto. Última oportunidad esta noche.

Reserva 1:1 ahora o espera a la próxima — $50 (30 min) · $100 (1 h).

🔴 Entrar a Main Stage 👉 ${MAIN_STAGE_URL}
❖ Reservar 1:1 👉 ${BOOK_URL}`;
  }
  return `🔴 @Dejesusof22 · FINAL CALL

Signing off soon. Last chance tonight.

Book 1:1 now or wait for next time — $50 (30 min) · $100 (1 h).
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `🔴 <b>@Dejesusof22 · ÚLTIMO LLAMADO</b>\n\n` +
      `Se despide pronto. Última oportunidad esta noche.\n\n` +
      `<b>Reserva 1:1 ahora</b> o espera a la próxima — $50 (30 min) · $100 (1 h).\n\n` +
      `Entra ya 👇`
    );
  }
  return (
    `🔴 <b>@Dejesusof22 · FINAL CALL</b>\n\n` +
    `Signing off soon. Last chance tonight.\n\n` +
    `<b>Book 1:1 now</b> or wait for next time — $50 (30 min) · $100 (1 h).\n\n` +
    `Jump in 👇`
  );
}
...
const TG_BUTTONS = {
  en: [[
    ...
    { text: '❖ Book 1:1',        url: BOOK_URL },
  ]],
  es: [[
    { text: '🔴 Entrar a Main Stage', url: MAIN_STAGE_URL },
    { text: '❖ Reservar 1:1',        url: BOOK_URL },
  ]],
};

const PUSH_COPY = {
  es: { title: '🔴 @Dejesusof22 · ÚLTIMO LLAMADO', body: 'Se despide pronto · reserva 1:1 desde $50' },
  en: { title: '🔴 @Dejesusof22 · FINAL CALL',     body: 'Signing off soon · book 1:1 from $50' },
};

const PUSH_COMMON = {
  url : MAIN_STAGE_URL,
  icon: '/icon-192.png',
  tag : 'dejesusof22-wave5-2026-09-04',
};
