...
/**
 * broadcast-santino-andres-calls-w3-20260925.js
 *
 ...
 * Usage:
 *   # dry-run
 *   docker cp apps/backend/scripts/broadcast-santino-andres-calls-w3-20260925.js pnptv-bot:/tmp/
 *   docker exec pnptv-bot node /tmp/broadcast-santino-andres-calls-w3-20260925.js --dry-run
 *
 *   # live — isolated container, NOT docker exec pnptv-bot
 *   docker run -d --name santino-andres-w3-blast \
 *     --env-file /tmp/bot-env.txt \
 *     --network pnptvapp_pnptvapp_net \
 *     -v /opt/pnptvapp/apps/backend/scripts/broadcast-santino-andres-calls-w3-20260925.js:/app/apps/backend/scripts/broadcast-santino-andres-calls-w3-20260925.js:ro \
 *     -v /root/promos/andres1.mp4:/tmp/andres1.mp4:ro \
 *     -v /root/promos/andres2.mp4:/tmp/andres2.mp4:ro \
 *     pnptv-bot:latest \
 *     node /app/apps/backend/scripts/broadcast-santino-andres-calls-w3-20260925.js
 */
...
const CAMPAIGN      = 'santino-andres-calls-w3-20260925';
...
const TG_CAPTION = {
  en: (name) => `🚨 <b>Santino + Andres — last hour, right now</b>
...
👉 <a href="${PROFILE_URL}">Book your session now →</a>`.trim(),
  ...
  es: (name) => `🚨 <b>Santino + Andres — última hora, ahora mismo</b>
...
async function main() {
  console.log('\n══════════════════════════════════════════════════════');
  console.log(` SANTINO + ANDRES CALLS BLAST — WAVE 3 (LAST HOUR) — ${CAMPAIGN}`);
  console.log('══════════════════════════════════════════════════════');
