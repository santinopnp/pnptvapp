...
/**
 * broadcast-nowpayments-wave.js
 *
 * Generic NowPayments wave — designed to run multiple times per day.
 * Sends a personalized $50/year checkout link to every non-PRIME user with Telegram.
 *
 * Invoice reuse: if a pending yearly50 NowPayments order already exists for the
 * user (from today or yesterday), that invoice_url is reused. No duplicate orders.
 *
 * Requires env var: WAVE_ID  (e.g. "20260921-1400")
 * BATCH_ID = "np-wave-${WAVE_ID}" — unique per wave so each wave is a fresh send.
 *
 * Usage:
 *   docker run ... -e WAVE_ID=20260921-1400 node apps/backend/scripts/broadcast-nowpayments-wave.js
...
const BATCH_ID    = `np-wave-${WAVE_ID}`;
...
function tgMsg(firstName, invoiceUrl, lang) {
  ...
  if (isEs) {
    return (
      `🔥 <b>Un año de PRIME por $50 — tu link de pago personal.</b>\n\n` +
      `Hola${name}, el precio especial de $50 por año sigue activo.\n\n` +
      `Pagá con cualquier cripto (USDC, BTC, ETH, Dash, LTC + más de 100 opciones):\n\n` +
  ...
  return (
    `🔥 <b>A full year of PRIME for $50 — your personal checkout link.</b>\n\n` +
    `Hi${name}, the $50/year special price is still available.\n\n` +
...
function pushPayload(lang) {
  const isEs = typeof lang === 'string' && lang.toLowerCase().startsWith('es');
  if (isEs) {
    return { title: '🔥 Un año de PRIME por $50 — tu link personal', body: 'Pagá con cripto. Acceso inmediato.', url: `${WEBAPP_URL}/subscribe`, tag: BATCH_ID, notifType: 'promo' };
  }
  return { title: '🔥 A full year of PRIME for $50 — your personal link', body: 'Pay with crypto. Instant access.', url: `${WEBAPP_URL}/subscribe`, tag: BATCH_ID, notifType: 'promo' };
}
...
async function tgSend(chatId, text) {
  return _tgApi('sendMessage', { chat_id: chatId, text, parse_mode: 'HTML', disable_web_page_preview: true });
}
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log(`  NowPayments wave  WAVE_ID=${WAVE_ID}  BATCH=${BATCH_ID}`);
  console.log(`  Plan     : ${PLAN_ID} @ $${PLAN_AMOUNT} | env=${process.env.NOWPAYMENTS_ENVIRONMENT || 'production'}`);
  ...
  if (DRY_RUN) {
    ...
    console.log('── EN ──'); console.log(tgMsg('Alex', fakeUrl, 'en'));
    console.log('\n── ES ──'); console.log(tgMsg('Alex', fakeUrl, 'es'));
