...
/**
 * broadcast-payment-launch-day2.js
 *
 * DAY 2 of the payment-system launch drip.
 * Angle: "60-second Ru$h 💎 tutorial — how to top up in a minute."
 * CTA:  /marketing/rush-tutorial-vertical.html
 *
 * Channels: payment_launch_day2_dm, payment_launch_day2_tg,
 *           payment_launch_day2_push_es, payment_launch_day2_push_en.
 * Dedup table: broadcast_payment_launch_day2.
 *
 * Fire from an isolated docker container (NOT `docker exec pnptv-bot`):
 *   docker exec pnptv-bot printenv > /tmp/broadcast-day2.env
 *   docker run --rm --name pnptv-broadcast-day2-live \
 *     --network pnptvapp_pnptvapp_net \
 *     -v /opt/pnptvapp/apps/backend/scripts/broadcast-payment-launch-day2.js:/app/apps/backend/scripts/broadcast-payment-launch-day2.js:ro \
 *     --env-file /tmp/broadcast-day2.env \
 *     pnptv-bot:latest \
 *     node apps/backend/scripts/broadcast-payment-launch-day2.js --live
 */
...
const TUTORIAL_URL   = 'https://pnptv.app/marketing/rush-tutorial-vertical.html';
...
const LOG_TABLE   = 'broadcast_payment_launch_day2';
const DM_CHANNEL  = 'payment_launch_day2_dm';
const TG_CHANNEL  = 'payment_launch_day2_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `💎 Ru$h en 60 segundos.

Recarga tu wallet una vez → gasta en propinas, videos exclusivos, llamadas privadas. Sin esperas, sin fricción.

Mira cómo 👇
${TUTORIAL_URL}`;
  }
  return `💎 Ru$h in 60 seconds.

Top up your wallet once → spend on tips, exclusive videos, private calls. No waits, no friction.
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `💎 <b>Ru$h en 60 segundos.</b>\n\n` +
      `Recarga tu wallet una vez → gasta en <b>propinas, videos exclusivos, llamadas privadas</b>. Sin esperas.\n\n` +
      `Mira cómo 👇`
    );
  }
  return (
    `💎 <b>Ru$h in 60 seconds.</b>\n\n` +
    `Top up your wallet once → spend on <b>tips, exclusive videos, private calls</b>. No waits.\n\n` +
    `See how 👇`
  );
}
...
const TG_BUTTONS = {
  en: [[
    { text: '💎 Watch (60 sec)', url: TUTORIAL_URL },
    { text: 'See all →',         url: MARKETING_URL },
  ]],
  es: [[
    { text: '💎 Ver (60 seg)', url: TUTORIAL_URL },
    { text: 'Ver todo →',      url: MARKETING_URL },
  ]],
};

const PUSH_COPY = {
  es: { title: '💎 Ru$h en 60 seg', body: 'Recarga una vez, gasta cuando quieras' },
  en: { title: '💎 Ru$h in 60 sec', body: 'Top up once, spend whenever' },
};

const PUSH_COMMON = {
  url : TUTORIAL_URL,
  icon: '/icon-192.png',
  tag : 'payment-launch-day2',
};
...
async function sendPushBatch(langKey) {
  ...
  const channel = `payment_launch_day2_push_${langKey}`;
  ...
  const { rows } = await query(`
    SELECT DISTINCT u.id::text AS id
      FROM users u JOIN push_subscriptions ps ON ps.user_id = u.id
     WHERE COALESCE(u.is_active, true) = true
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — Payment System Launch · Day 2 (Buy Ru$h tutorial)');
  console.log(`  MODE     : ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log(`  Hero     : ${VIDEO_URL}`);
  console.log(`  CTA      : ${TUTORIAL_URL}`);
  console.log('═══════════════════════════════════════════════════════════════\n');
  ...
  console.log(`  ${targets.length} users total   ${tgEligible.length} TG eligible   Santino: ${includesSantino ? 'YES' : 'NO'}\n`);
