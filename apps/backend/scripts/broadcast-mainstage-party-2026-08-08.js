...
/**
 * Main Stage party broadcast — 2026-08-08 (9:30 PM Colombia)
 * Push + Telegram DM to ALL active users. Bilingual (ES/EN by user.language).
 * Manual one-shot only — do NOT crontab.
 *
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-mainstage-party-2026-08-08.js --dry-run
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-mainstage-party-2026-08-08.js
 */
...
const COPY = {
  es: {
    push: {
      title: '🔥 HOY 9:30 PM COL — Party en Main Stage',
      body: 'Santino, SOTUME, Lex y Deus prendidos 🥵 cárgate y reserva tu privado',
    },
    tg: `💊🔥 HOY 9:30 PM Colombia — Santino, SOTUME, Lex y Deus prendidos en Main Stage.
...
— PNPtv!`,
  ...
  en: {
    push: {
      title: '🔥 TONIGHT 9:30 PM COL — Party on Main Stage',
      body: 'Santino, SOTUME, Lex & Deus lit 🥵 get loaded & book your private',
    },
    tg: `💊🔥 TONIGHT 9:30 PM Colombia (UTC-5) — Santino, SOTUME, Lex & Deus lit on Main Stage.

Get loaded & book your private 🥵
...
const PUSH_COMMON = {
  url: '/main-stage',
  icon: '/icon-192.png',
  tag: 'mainstage-party-2026-08-08',
};
...
async function ensureLogTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS broadcast_mainstage_party_2026_08_08 (
...
async function alreadySent(userId, channel) {
  const { rows } = await query(
    `SELECT 1 FROM broadcast_mainstage_party_2026_08_08 WHERE user_id=$1 AND channel=$2 AND status='sent'`,
...
async function log(userId, channel, status, error) {
  await query(
    `INSERT INTO broadcast_mainstage_party_2026_08_08 (user_id, channel, status, error)
     VALUES ($1,$2,$3,$4)
...
async function sendTelegramDMs() {
  ...
  for (const u of rows) {
    ...
    // Throttle: ~25 msg/sec cap → 40ms between sends, plus 1s breather every 10 msgs
    await sleep(40);
    if ((sent + fail) % 100 === 0) {
      await sleep(1000);
...
async function main() {
  console.log(`=== Main Stage party broadcast 2026-08-08 ${DRY ? '(DRY-RUN)' : ''} ===`);
