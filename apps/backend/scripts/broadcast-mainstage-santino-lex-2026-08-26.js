...
/**
 * Main Stage broadcast — 2026-08-26 (Santino + Lex + 2 boys LIVE)
 * Push + Telegram DM to ALL active users. Bilingual (ES/EN by user.language).
 * Manual one-shot only — do NOT crontab.
 *
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-mainstage-santino-lex-2026-08-26.js --dry-run
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-mainstage-santino-lex-2026-08-26.js
 */
...
const COPY = {
  es: {
    push: {
      title: '🔥 AHORA EN VIVO — Main Stage',
      body: 'Santino, Lex y 2 chicos prendidos 🥵 entra',
    },
    tg: `💊🔥 AHORA en Main Stage — Santino, Lex y 2 chicos prendidos 🥵

Entra, cárgate y tira tus Ru$h 💎 o reserva tu privado.
...
— PNPtv!`,
  ...
  en: {
    push: {
      title: '🔥 LIVE NOW — Main Stage',
      body: 'Santino, Lex & 2 boys going hard 🥵 come in',
    },
    tg: `💊🔥 LIVE NOW on Main Stage — Santino, Lex + 2 boys going hard 🥵

Come get loaded. Drop your Ru$h 💎 or book your private.

👉 ${CTA_URL}

— PNPtv!`,
    btn: '🎥 Go to Main Stage',
  },
};
...
const PUSH_COMMON = {
  url: '/main-stage',
  icon: '/icon-192.png',
  tag: 'mainstage-santino-lex-2026-08-26',
};

const sleep = (ms) => new Promise(r => setTimeout(r, ms));

function tgSend(chatId, text, btnLabel) {
  return new Promise((resolve) => {
    const payload = JSON.stringify({
      ...
      reply_markup: {
        inline_keyboard: [[{ text: btnLabel, url: CTA_URL }]],
      },
    });
...
async function ensureLogTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS broadcast_mainstage_santino_lex_2026_08_26 (
...
async function alreadySent(userId, channel) {
  const { rows } = await query(
    `SELECT 1 FROM broadcast_mainstage_santino_lex_2026_08_26 WHERE user_id=$1 AND channel=$2 AND status='sent'`,
...
async function log(userId, channel, status, error) {
  await query(
    `INSERT INTO broadcast_mainstage_santino_lex_2026_08_26 (user_id, channel, status, error)
     VALUES ($1,$2,$3,$4)
...
async function sendTelegramDMs() {
  ...
  for (const u of rows) {
    ...
    const btn  = isEs ? COPY.es.btn : COPY.en.btn;
    ...
    const r = await tgSend(u.telegram, text, btn);
...
async function main() {
  console.log(`=== Main Stage Santino+Lex broadcast 2026-08-26 ${DRY ? '(DRY-RUN)' : ''} ===`);
