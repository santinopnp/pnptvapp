...
const LOG_TABLE   = 'broadcast_dejesusof22_wave3_2026_09_04';
const DM_CHANNEL  = 'dejesusof22_wave3_dm';
const TG_CHANNEL  = 'dejesusof22_wave3_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `🔴 @Dejesusof22 · HORA PRIME

Audiencia peak en Main Stage ahora mismo.

Llévalo privado antes de que se llene — $50 (30 min) · $100 (1 h).

🔴 Entrar a Main Stage 👉 ${MAIN_STAGE_URL}
❖ Reservar 1:1 👉 ${BOOK_URL}`;
  }
  return `🔴 @Dejesusof22 · PRIME HOUR

Peak audience on Main Stage right now.

Take him private before the slot fills — $50 (30 min) · $100 (1 h).
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `🔴 <b>@Dejesusof22 · HORA PRIME</b>\n\n` +
      `Audiencia peak en Main Stage ahora mismo.\n\n` +
      `Llévalo <b>privado antes de que se llene</b> — $50 (30 min) · $100 (1 h).\n\n` +
      `Reserva 👇`
    );
  }
  return (
    `🔴 <b>@Dejesusof22 · PRIME HOUR</b>\n\n` +
    `Peak audience on Main Stage right now.\n\n` +
    `Take him <b>private before the slot fills</b> — $50 (30 min) · $100 (1 h).\n\n` +
    `Book 👇`
  );
}
...
const PUSH_COPY = {
  es: { title: '🔴 @Dejesusof22 · HORA PRIME', body: 'Peak en Main Stage · reserva 1:1 desde $50' },
  en: { title: '🔴 @Dejesusof22 · PRIME HOUR', body: 'Peak on Main Stage · book 1:1 from $50' },
};

const PUSH_COMMON = {
  url : MAIN_STAGE_URL,
  icon: '/icon-192.png',
  tag : 'dejesusof22-wave3-2026-09-04',
};
