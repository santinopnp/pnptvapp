...
/**
 * broadcast-lifetime100-email-2026-08-20.js
 *
 * Week-of-Aug-19 campaign — Send 2/4.
 * Channel:  Email (Hostinger SMTP, per feedback_broadcast_no_resend + feedback_email_send_only)
 * Segment:  non-PRIME users with a real email (~1.6k)
 * Angle:    Weekend warm-up. Hero = /lifetime100. Small monthly fallback CTA below.
 * Idempotency: broadcast_lifetime100_week_2026_08_19 log table, channel=thu_email
 *
 * Usage:
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-lifetime100-email-2026-08-20.js
 *   docker exec pnptv-bot node /app/apps/backend/scripts/broadcast-lifetime100-email-2026-08-20.js --live
 */
...
function buildEmail(u) {
  ...
  const subject = es
    ? `PRIME de por vida por $100 — el fin de semana empieza hoy 🖤`
    : `Lifetime PRIME for $100 — the weekend starts today 🖤`;
  ...
  const bodyEs = `
<p>Hey ${name} 👋</p>
<p>El fin de semana está cerca — y una buena forma de arrancarlo es <strong>desbloquear PNPtv! para siempre</strong>.</p>
...
`;
  ...
  const bodyEn = `
<p>Hey ${name} 👋</p>
<p>Weekend's around the corner — and a good way to start it is by <strong>unlocking PNPtv! for good</strong>.</p>
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — /lifetime100 email broadcast (Thu 2026-08-20)');
  ...
  console.log(`  Found ${targets.length} user(s) with reachable email\n`);
  ...
  for (let i = 0; i < targets.length; i++) {
    const u = targets[i];
    if (await alreadySent(u.user_id, 'thu_email')) { skip++; continue; }
    ...
    try {
      ...
      if (r && r.success !== false) {
        sent++;
        await log(u.user_id, 'thu_email', 'sent');
      } else {
        fail++;
        await log(u.user_id, 'thu_email', 'failed', ((r && r.error) || '?').slice(0, 500));
      }
    } catch (e) {
      fail++;
      await log(u.user_id, 'thu_email', 'failed', (e.message || '?').slice(0, 500));
    }
