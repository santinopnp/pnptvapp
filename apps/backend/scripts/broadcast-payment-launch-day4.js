...
/**
 * broadcast-payment-launch-day4.js
 *
 * DAY 4 of the payment-system launch drip.
 * Angle: "Support your favorite creator monthly — one tap."
 * CTA:  /marketing/subscribe-creator.html
 *
 * Channels: payment_launch_day4_dm, payment_launch_day4_tg,
 *           payment_launch_day4_push_es, payment_launch_day4_push_en.
 * Dedup table: broadcast_payment_launch_day4.
 *
...
const CREATOR_URL    = 'https://pnptv.app/marketing/subscribe-creator.html';
...
const LOG_TABLE   = 'broadcast_payment_launch_day4';
const DM_CHANNEL  = 'payment_launch_day4_dm';
const TG_CHANNEL  = 'payment_launch_day4_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `💎 Apoya a tu creador favorito — mensual, sin drama.

Suscríbete a tu Crystal Creator. Contenido exclusivo, acceso privado, todo desde tu wallet.

Elige tu creador 👇
${CREATOR_URL}`;
  }
  return `💎 Back your favorite creator — monthly, no drama.

Subscribe to your Crystal Creator. Exclusive content, private access, all from your wallet.
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `💎 <b>Apoya a tu creador favorito.</b>\n\n` +
      `Suscríbete a tu <b>Crystal Creator</b>. Contenido exclusivo, acceso privado, todo desde tu wallet.\n\n` +
      `Elige tu creador 👇`
    );
  }
  return (
    `💎 <b>Back your favorite creator.</b>\n\n` +
    `Subscribe to your <b>Crystal Creator</b>. Exclusive content, private access, all from your wallet.\n\n` +
    `Pick your creator 👇`
  );
}
...
const TG_BUTTONS = {
  en: [[
    { text: '💎 Pick a Creator', url: CREATOR_URL },
    { text: 'See all →',         url: MARKETING_URL },
  ]],
  es: [[
    { text: '💎 Elige Creador', url: CREATOR_URL },
    { text: 'Ver todo →',       url: MARKETING_URL },
  ]],
};

const PUSH_COPY = {
  es: { title: '💎 Apoya a tu creador', body: 'Suscripción mensual · desde tu wallet' },
  en: { title: '💎 Back your creator',  body: 'Monthly subscription · from your wallet' },
};

const PUSH_COMMON = { url: CREATOR_URL, icon: '/icon-192.png', tag: 'payment-launch-day4' };
...
async function sendPushBatch(langKey) {
  ...
  const channel = `payment_launch_day4_push_${langKey}`;
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — Payment System Launch · Day 4 (Subscribe to Creator)');
  console.log(`  MODE  : ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log(`  Hero  : ${VIDEO_URL}`);
  console.log(`  CTA   : ${CREATOR_URL}`);
  console.log('═══════════════════════════════════════════════════════════════\n');
