...
/**
 * broadcast-yearly50-weekend-20260920.js
 *
 * Mass broadcast to all non-PRIME users promoting the $50/year PRIME plan.
 * Offer valid this weekend only (Sep 20–21 2026).
 *
 ...
 * Hero      : https://pnptv.app/promos/promo-1.mp4  (video)
 * Dedup     : broadcast_dedup LIKE 'yearly50-weekend-20260920%'
 *
 ...
 *     node apps/backend/scripts/broadcast-yearly50-weekend-20260920.js --dry-run
 *
...
const BATCH_ID   = 'yearly50-weekend-20260920';
...
function dmText(lang) {
  if (lang === 'es') {
    return `🔥 *Un año de PRIME por $50 — solo este fin de semana*

Soy Santino. Quería escribirte directamente.

Este fin de semana activás un año completo de PRIME por solo $50 — contenido exclusivo, Main Stage en vivo, llamadas privadas con creadores, y todo lo que seguimos construyendo. Sin cobros recurrentes.

La oferta vence el domingo.

👉 ${CTA_URL}`;
  }
  return `🔥 *A full year of PRIME for $50 — this weekend only*

Santino here. Wanted to reach you directly.

This weekend you can unlock a full year of PRIME for just $50 — exclusive content, the live Main Stage, private calls with creators, and everything we keep building. No recurring charges.
...
function pushPayload(lang) {
  if (lang === 'es') {
    return {
      title: '🔥 Un año de PRIME por $50 — solo este fin de semana',
      body:  'Sin cobros recurrentes. La oferta vence el domingo.',
      url:   CTA_URL,
      tag:   BATCH_ID,
      notifType: 'promo',
    };
  ...
  return {
    title: '🔥 A full year of PRIME for $50 — this weekend only',
    body:  'No recurring charges. Offer ends Sunday.',
    url:   CTA_URL,
    tag:   BATCH_ID,
    notifType: 'promo',
  };
...
function emailSubject(lang) {
  return lang === 'es'
    ? '🔥 Un año de PRIME por $50 — solo este fin de semana'
    : '🔥 A full year of PRIME for $50 — this weekend only';
}
...
function emailHtml(lang) {
  const preheader = lang === 'es'
    ? 'Un año de PRIME por $50 — la oferta vence el domingo'
    : 'A full year of PRIME for $50 — offer ends Sunday';
  const body = lang === 'es' ? `
    <p style="font-size:22px;font-weight:700;color:#fff;margin:0 0 16px">🔥 Solo este fin de semana</p>
    <p>Soy Santino, fundador de PNPtv. Quería escribirte directamente.</p>
    <p>Este fin de semana activás <strong>un año completo de PRIME por solo $50</strong> — sin cobros recurrentes, sin sorpresas. Acceso total: contenido exclusivo, Main Stage en vivo, llamadas privadas con creadores, y todo lo que seguimos construyendo.</p>
    ...
    <p style="font-size:13px;color:#aaa">La oferta vence el domingo 21 de septiembre.</p>
    <p>— Santino<br><span style="font-size:12px;color:#888">Fundador, PNPtv!</span></p>
  ` : `
    <p style="font-size:22px;font-weight:700;color:#fff;margin:0 0 16px">🔥 This weekend only</p>
    <p>I'm Santino, founder of PNPtv. Wanted to reach you directly.</p>
    <p>This weekend you can unlock <strong>a full year of PRIME for just $50</strong> — no recurring charges, no surprises. Full access: exclusive content, the live Main Stage, private calls with creators, and everything we keep building.</p>
    <div style="text-align:center;margin:32px 0">
      <a href="${CTA_URL}" style="display:inline-block;padding:16px 32px;background:linear-gradient(90deg,#d4007a,#ff6600);color:#fff;font-weight:800;font-size:16px;border-radius:12px;text-decoration:none;letter-spacing:0.04em">
        💎 SEE ANNUAL PLAN — $50
      </a>
    </div>
    <p style="font-size:13px;color:#aaa">Offer ends Sunday, September 21st.</p>
...
async function main() {
  ...
  const { rows: dedupRows } = await query(
    ...
    ['yearly50-weekend-20260920%']
  );
  const alreadySent = parseInt(dedupRows[0].cnt, 10);

  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — PRIME Annual $50 · Weekend promo broadcast');
  ...
  for (const u of users) {
    ...
    const { rows: already } = await query(
      ...
      ['yearly50-weekend-20260920%', u.id]
    );
    if (already.length > 0) {
      skipped++;
      continue;
