...
/**
 * broadcast-payment-launch-day5.js
 *
 * DAY 5 of the payment-system launch drip.
 * Angle: "Book any creator — on your terms."
 * CTA:  /marketing/book-a-call.html
 *
 * Channels: payment_launch_day5_dm, payment_launch_day5_tg,
 *           payment_launch_day5_push_es, payment_launch_day5_push_en.
 * Dedup table: broadcast_payment_launch_day5.
 *
...
const BOOK_URL       = 'https://pnptv.app/marketing/book-a-call.html';
...
const LOG_TABLE   = 'broadcast_payment_launch_day5';
const DM_CHANNEL  = 'payment_launch_day5_dm';
const TG_CHANNEL  = 'payment_launch_day5_tg';
...
function dmText(lang) {
  if (isEs(lang)) {
    return `💎 Reserva una llamada — a tu ritmo.

Elige creador, elige duración, paga con wallet o Ru$h 💎. Cara a cara, cuando tú quieras.

Reserva ahora 👇
${BOOK_URL}`;
  }
  return `💎 Book a call — at your pace.

Pick a creator, pick a length, pay with wallet or Ru$h 💎. Face-to-face, when you want it.
...
function tgCaption(lang) {
  if (isEs(lang)) {
    return (
      `💎 <b>Reserva una llamada — a tu ritmo.</b>\n\n` +
      `Elige creador, elige duración, paga con <b>wallet o Ru$h 💎</b>. Cara a cara, cuando tú quieras.\n\n` +
      `Reserva ahora 👇`
    );
  }
  return (
    `💎 <b>Book a call — at your pace.</b>\n\n` +
    `Pick a creator, pick a length, pay with <b>wallet or Ru$h 💎</b>. Face-to-face, when you want it.\n\n` +
    `Book now 👇`
  );
}
...
const TG_BUTTONS = {
  ...
  es: [[
    { text: '💎 Reservar', url: BOOK_URL },
    { text: 'Ver todo →',  url: MARKETING_URL },
  ]],
};

const PUSH_COPY = {
  es: { title: '💎 Reserva una llamada', body: 'Cara a cara · paga con wallet' },
  en: { title: '💎 Book a call',         body: 'Face-to-face · pay with wallet' },
};

const PUSH_COMMON = { url: BOOK_URL, icon: '/icon-192.png', tag: 'payment-launch-day5' };
...
async function sendPushBatch(langKey) {
  ...
  const channel = `payment_launch_day5_push_${langKey}`;
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PNPtv — Payment System Launch · Day 5 (Book a Call)');
  console.log(`  MODE  : ${DRY ? 'DRY RUN' : 'LIVE'}`);
  console.log(`  Hero  : ${VIDEO_URL}`);
  console.log(`  CTA   : ${BOOK_URL}`);
  console.log('═══════════════════════════════════════════════════════════════\n');
