  ...
  try {
    ...
    return res.json({
      ...
      profile: {
        ...
        pnptvFamBenefitsPending: !!p.is_pnptv_fam && !p.pnptv_fam_benefits_seen_at,
        pnptvFamFeedLayout: p.pnptv_fam_feed_layout || { mode: 'fam', shortcuts: [] },
...
const updateProfile = async (req, res) => {
  ...
  if (dateOfBirth !== undefined && dateOfBirth !== null && dateOfBirth !== '') {
    ...
    if (age < 18) return res.status(400).json({ error: 'AGE_REQUIREMENT_NOT_MET', message: 'You must be 18 or older to use this platform.' });
