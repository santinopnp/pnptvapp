...
class HypeBotScheduler {
  ...
  async _emitHypeMessages() {
    ...
    // Look up creator hype_bot_enabled + language for eligible streams.
    // The room / redis viewers key is whatever the client sent to live:join.
    // In practice that is a Restreamer channel slug matched against
    // users.live_channel (e.g. "pnptv-lex"). live_streams is legacy scheduling
    // metadata — its `status` column only ever holds 'ended', so a UUID-branch
    // join there would never match a live show. Rely on Redis viewer presence
    // (≥2 viewers) as the liveness signal; the scan above already filtered by it.
    let dbRows = [];
    try {
      const { rows } = await query(
        `SELECT u.live_channel AS stream_id,
                u.hype_bot_enabled,
